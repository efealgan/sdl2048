...
bool initText (){
    ...
    //as font is now open, rendering possible tile values should be done here.
    int boardSize = 4;
    
    for (int i = 1; i < pow(boardSize , 2) + 1; i++) {
        ...
        if (text) {
            ...
            SDL_Log("Couldn't render text surface: %s\n", SDL_GetError());
            return 1;
        }
    }  
    return 0;
...
void drawText (SDL_Renderer *renderer, int targetX, int targetY, int textureIndex){
    ...
    dst.x = ((float(tileSizePixels)/2) - dst.w/2) + targetX;
    dst.y = ((float(tileSizePixels)/2) - dst.h/2) + targetY;
    //dst.x = (w - dst.w) / 2;
    //dst.y = (h - dst.h) / 2;

    bool drawTextDebug = 0;
    if(drawTextDebug) {
